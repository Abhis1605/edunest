...
const ReportDownloadButton = dynamic(
  () => import("@/components/shared/ReportDownloadButton"),
  {
    ssr: false,
    loading: () => (
      <button
...
export default function StudentReportPage() {
  ...
  const fetchReport = async () => {
    ...
    try {
      const data = await api.get(
        `/api/student/report?from=${fromDate}&to=${toDate}`,
      );
      if (!data.subjects || data.subjects.length === 0) {
        toast.error(`No exam data found from ${fromDate} to ${toDate}`);
        setReport(null);
        return;
      }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">My Report</h1>
        <p className="text-muted-foreground mt-1">
          Select a date range and download your progress report
        </p>
      </div>
    ...
    justify-between gap-4 p-4 bg-card border border-border rounded-xl">
        <div>
