...
const styles = StyleSheet.create({
  ...
  // attendance row — clean no color boxes
  attendanceRow: {
    flexDirection: "row",
    border: `1 solid ${BORDER}`,
    borderRadius: 5,
  ...
  return (
    <Svg width={W} height={H}>
      ...
      {safeSubjects.map((s, i) => {
        ...
        const label =
          s.subject.length > 8 ? s.subject.slice(0, 8) + "…" : s.subject;
      ...
      <Text x={PAD.left + 8} y={H - 5} fontSize={5} fill={GRAY}>
        Student %
      </Text>
      <Rect
        x={PAD.left + 58}
        ...
        rx={1}
      />
      <Text x={PAD.left + 66} y={H - 5} fontSize={5} fill={GRAY}>
        Class Highest %
      </Text>
    </Svg>
  );
...
// ── MAIN DOCUMENT ─────────────────────────────────────────────────────────────
export function StudentReportDocument({ report }) {
  const { student, attendance, subjects, generatedAt } = report;

  return (
    <Document title={`Report - ${student?.name}`} author="EduNest">
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.body}>
          ...
          {/* Clean attendance row — no color boxes */}
          <View style={styles.attendanceRow}>
            {[
              ...
              <View
                key={label}
                style={idx === 3 ? styles.attCellLast : styles.attCell}
              >
              ...
              <View style={styles.table}>
                ...
                {subjects.map((s, i) => [
                  ...s.exams.map((e, j) => {
                    const pct =
                      e.maxMarks > 0
                        ? ((e.marks / e.maxMarks) * 100).toFixed(1)
                        : 0;
...
>
    {j === 0 ? s.subject : ""}
                        ...
                        >
                          <View
                            style={[styles.gradeBadge, { backgroundColor: bg }]}
                          >
                            <Text
                              style={{
                                fontSize: 6.5,
                    ...
                    >
                      <View
                        style={[
                          styles.gradeBadge,
                          {
                            backgroundColor: getGrade(Number(s.percentage)).bg,
                          },
                        ]}
                      >
                        <Text
                          style={{
                            fontSize: 6.5,
                            fontFamily: "Helvetica-Bold",
                            color: getGrade(Number(s.percentage)).color,
                          }}
                        >
        ...
        <View style={styles.signatureRow} fixed>
          {["Student Signature", "Class Teacher", "Parent Signature"].map(
    (label, i) => (
        ...
        <View style={styles.footer} fixed>
          <Text style={styles.footerText}>
            EduNest — Student Management Portal
          </Text>
